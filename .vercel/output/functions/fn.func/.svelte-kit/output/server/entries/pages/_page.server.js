...
async function load() {
  ...
  if (!podcastVideosResponse.ok)
    return "Error with podcast videos";
  if (!articleCategoriesResponse.ok)
    return "Error with articles categories";
  if (!galleryImageResponse.ok)
    return "Error with articles categories";
  if (!printMediaResponse.ok)
    return "Error with print media categories";
  if (!incomingSessionResponse.ok)
    return "Error with live sessions fetching";
  if (!bookURLResponse.ok)
    return "Error with loading book image urls";
